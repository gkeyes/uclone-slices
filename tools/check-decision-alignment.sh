...
FIXTURE_VERSION=$(sed -n 's/^[[:space:]]*versionName = "\(.*\)"/\1/p' "$ROOT/device-fixture-app/build.gradle.kts")
...
FIXTURE_VERSION_CODE=$(sed -n 's/^[[:space:]]*versionCode = \([0-9][0-9]*\)/\1/p' "$ROOT/device-fixture-app/build.gradle.kts")
