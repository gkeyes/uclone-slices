...
grep -F 'for script in customize.sh post-fs-data.sh emergency-containment.sh startup-gate.sh service.sh boot-completed.sh boot-state.sh rescue.sh' "$PACKAGER" >/dev/null || fail 'package script list is incomplete'
...
for script in post-fs-data.sh emergency-containment.sh startup-gate.sh service.sh boot-completed.sh rescue.sh; do
