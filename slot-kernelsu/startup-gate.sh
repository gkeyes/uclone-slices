...
while pending_for_boot; do
    ...
    result="$("$TOYBOX_BIN" timeout -s 9 30 "$TOYBOX_BIN" nsenter -t 1 -m -- \
        "$RUNTIME_BIN" --startup-gate 2>>"$LOG_FILE")"
    status=$?
    if [ "$status" -eq 0 ]; then
        case "$result" in
            held|not-managed)
                if publish_ready "$result"; then
                    write_log "startup gate proved for boot $boot_id: $result"
                    exit 0
                fi
                ;;
        esac
    fi
    write_log "startup gate pending for boot $boot_id; status=$status"
    "$TOYBOX_BIN" sleep "$delay"
    [ "$delay" -ge 5 ] || delay=$((delay + 1))
done
