...
static uint32_t load_big_endian(const uint8_t *input) {
    return ((uint32_t)input[0] << 24U) | ((uint32_t)input[1] << 16U) |
           ((uint32_t)input[2] << 8U) | (uint32_t)input[3];
}

static void store_big_endian(uint8_t *output, uint32_t value) {
...
void sha256_update(struct sha256_context *context, const uint8_t *data, size_t length) {
    ...
    while (length != 0U) {
        const size_t available = sizeof(context->block) - context->block_used;
...
void sha256_final(struct sha256_context *context, uint8_t digest[SHA256_DIGEST_BYTES]) {
    ...
    if (context->block_used > 56U) {
        memset(context->block + context->block_used, 0, sizeof(context->block) - context->block_used);
        transform(context, context->block);
        context->block_used = 0U;
    }
    ...
    transform(context, context->block);
    for (size_t index = 0U; index < 8U; ++index) {
        store_big_endian(digest + (index * 4U), context->state[index]);
    }
