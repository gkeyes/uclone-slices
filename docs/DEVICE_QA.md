...
2. 安装 0.1.9 模块并重启，直接发送与 0.1.8 相同的无 `client_build_id` `list/get` 和不存在槽位写请求：必须返回 `invalid_request`，日志保留版本门禁记录，账号状态不变。Android 17 不允许正式 APK 覆盖降级；不得为安装旧 Manager 而卸载或清数据。
...
3. 全部门禁通过且用户明确确认可以发布后，才创建或更新 GitHub Release；失败时执行第 1 节保存的覆盖回退，不做数据迁移。
