...
- [ ] 评估 Manager 进程被系统直接杀死、但设备未重启时的中断事务提示与显式收敛入口。
  - 当前前台 Service 正常取消会执行 `finish/abort`，Runtime 开机时也会在普通挂载收敛前恢复 account-I/O intent。
  - 0.2.0 已提供只读 `list_account_io_status`，但不在没有用户确认的情况下自动中断来自其他 root 客户端的事务。
...
- 0.2.0 wire 为十八个命令和十一个错误码；变更必须同时具备用例、fixture、Kotlin 消费和 UI。
