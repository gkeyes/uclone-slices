...
use uclone_slot_runtime::protocol::{Command, Request, RequestId};
...
fn compiled_constants_match_the_selected_fixed_profile() {
    match PROFILE {
        ...
        "generic" => assert_eq!(PACKAGE, "com.uclone.slots.preview"),
...
#[test]
fn protocol_accepts_valid_packages_and_runtime_issued_slots() {
    for package in [
        target_package(),
        ...
        for slot in [
            SlotId::base(),
            SlotId::parse(PREVIEW_SLOT).unwrap(),
            SlotId::parse("slot-123").unwrap(),
        ] {
